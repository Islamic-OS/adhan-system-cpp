...
bool is_valid_time(time_components_t time_components) {
  ...
  return !(time_components.hours == invalid_time_t.hours &&
...
inline time_components_t from_double(double value) {
  ...
  const int seconds =
      (int)floor((value - (hours + minutes / 60.0)) * 60.0 * 60.0);
  time_components = (time_components_t){hours, minutes, seconds};
  return time_components;
}

extern time_components_t new_time_components(int hours, int minutes,
                                             int seconds) {
  time_components_t time_components = {hours, minutes, seconds};
  return time_components;
}

time_t get_tm_date(time_components_t *time_components,
                   date_components_t *date_components) {
...
inline date_components_t from_time_t(const time_t time) {
  ...
  date_components_t date_components = {date->tm_mday, date->tm_mon + 1,
                                       date->tm_year + 1900};
  return date_components;
}
...
time_t resolve_time(const date_components_t *date_components) {
  return resolve_time_2(date_components->year, date_components->month,
                        date_components->day);
}
...
time_t get_date_components(const time_t time,
                           const time_components_t *time_components) {
