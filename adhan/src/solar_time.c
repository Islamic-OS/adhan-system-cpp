...
solar_time_t new_solar_time(const time_t today_time,
  ...
  solar_coordinates_t solar =
      new_solar_coordinates(julianDay2(localtime(&today_time)));
  solar_coordinates_t prevSolar =
      new_solar_coordinates(julianDay2(localtime(&yesterday_time)));
  solar_coordinates_t nextSolar =
      new_solar_coordinates(julianDay2(localtime(&tomorrow_time)));

  double approximateTransit = getApproximateTransit(
  ...
  double sunrise = correctedHourAngle(
  ...
  double sunset = correctedHourAngle(
  ...
  solar_time_t result =
      (solar_time_t){transit, sunrise,   sunset,    observer,
                     solar,   prevSolar, nextSolar, approximateTransit};

  return result;
}

double hourAngle(solar_time_t *this_solar_time, double angle,
                 bool afterTransit) {
  return correctedHourAngle(this_solar_time->approximateTransit, angle,
                            this_solar_time->observer, afterTransit,
                            this_solar_time->solar.apparentSiderealTime,
                            this_solar_time->solar.rightAscension,
                            this_solar_time->prevSolar.rightAscension,
                            this_solar_time->nextSolar.rightAscension,
                            this_solar_time->solar.declination,
                            this_solar_time->prevSolar.declination,
                            this_solar_time->nextSolar.declination);
}
...
double afternoon(solar_time_t *this_solar_time, shadow_length shadowLength) {
  ...
  const double tangent = fabs(this_solar_time->observer->latitude -
                              this_solar_time->solar.declination);
