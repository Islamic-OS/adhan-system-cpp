...
solar_coordinates_t new_solar_coordinates(double julianDay) {
  ...
  double delta_psi =
      nutationInLongitude(/* julianCentury */ T, /* solarLongitude */ L0,
                          /* lunarLongitude */ Lp, /* ascendingNode */ omega);
  double delta_epsilon =
      nutationInObliquity(/* julianCentury */ T, /* solarLongitude */ L0,
                          /* lunarLongitude */ Lp, /* ascendingNode */ omega);
  ...
  double apparentSiderealTime =
      theta0 +
      (((delta_psi * 3600) * cos(to_radius(epsilon0 + delta_epsilon))) / 3600);

  solar_coordinates_t solar_coordinates =
      (solar_coordinates_t){declination, rightAscension, apparentSiderealTime};
