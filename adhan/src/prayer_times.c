...
inline prayer_times_t new_prayer_times(coordinates_t *coordinates,
                                       date_components_t *date,
                                       calculation_parameters_t *params) {
  return new_prayer_times2(coordinates, resolve_time(date), params);
}

prayer_times_t new_prayer_times2(coordinates_t *coordinates, time_t date,
  ...
  time_t transit = is_valid_time(time_components)
                       ? get_date_components(date, &time_components)
                       : 0;

  time_components = from_double(solar_time.sunrise);
  time_t sunriseComponents = is_valid_time(time_components)
                                 ? get_date_components(date, &time_components)
                                 : 0;

  time_components = from_double(solar_time.sunset);
  time_t sunsetComponents = is_valid_time(time_components)
                                ? get_date_components(date, &time_components)
                                : 0;
  ...
  if (!error) {
    ...
    time_components = from_double(
        afternoon(&solar_time, getShadowLength(parameters->madhab)));
    ...
    time_components =
        from_double(hourAngle(&solar_time, -parameters->fajrAngle, false));
    if (is_valid_time(time_components)) {
    ...
    if (parameters->method == MOON_SIGHTING_COMMITTEE &&
      ...
      time_t tmp_time =
          add_seconds(sunriseComponents, -1 * (int)(night / 7000));
      tempFajr = tmp_time;
    }
    ...
    if (parameters->ishaInterval > 0) {
      ...
      time_components =
          from_double(hourAngle(&solar_time, -parameters->ishaAngle, true));
      if (is_valid_time(time_components)) {
  ...
  } else if (parameters->method == UMM_AL_QURA || parameters->method == GULF ||
  ...
  if (parameters->method == MOON_SIGHTING_COMMITTEE) {
    // Moonsighting Committee adds 3 minutes to sunset time to account for light
    // refraction
  ...
  if (error || !tempAsr) {
    ...
    time_t final_fajr_time =
        round_minute(add_minutes(tempFajr, parameters->adjustments.fajr));
    time_t final_sunrise_time =
        round_minute(add_minutes(tempSunrise, parameters->adjustments.sunrise));
    time_t final_dhuhr_time = round_minute(add_minutes(
        tempDhuhr, parameters->adjustments.dhuhr + dhuhrOffsetInMinutes));
    time_t final_asr_time =
        round_minute(add_minutes(tempAsr, parameters->adjustments.asr));
    time_t final_maghrib_time = round_minute(add_minutes(
        tempMaghrib, parameters->adjustments.maghrib + maghribOffsetInMinutes));
    time_t final_isha_time =
        round_minute(add_minutes(tempIsha, parameters->adjustments.isha));

    prayer_times_t prayer_times = {final_fajr_time,    final_sunrise_time,
                                   final_dhuhr_time,   final_asr_time,
                                   final_maghrib_time, final_isha_time};
    return prayer_times;
  }
}

prayer_times_t new_prayer_times_with_tz(coordinates_t *coordinates, time_t date,
                                        calculation_parameters_t *parameters,
                                        int tz) {
  prayer_times_t prayer_times =
      new_prayer_times2(coordinates, date, parameters);
